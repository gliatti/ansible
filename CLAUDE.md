...
**OS-family dispatch.** Cross-distro roles (postgresql, ldap2pg) branch with `include_tasks: '{{ ansible_facts["os_family"] }}/install.yml'` into `tasks/Debian/` and `tasks/RedHat/` subdirectories, with RedHat-specific vars in `vars/RedHat.yml` (postgresql, pgbouncer). Facts are always read through `ansible_facts[...]`, never as injected top-level variables (removed in ansible-core 2.24). The PGDG repository lives in `roles/postgresql/tasks/repository_manager.yml` and is reused by the pgbouncer and barman roles (`include_role` + `tasks_from`), because on RedHat both packages only exist in `pgdg-common`. Both families must be kept working (the vagrant setup has one Debian 12 and one Rocky 9 VM for exactly this).

**Variables.** All role variables are prefixed with the role name (`postgresql_*`, `barman_*`, …) and live in `defaults/main.yml`, which is the reference documentation: defaults are split into "don't override" and "can be overridden" sections, and empty list vars (`postgresql_roles`, `postgresql_privileges`) carry commented examples showing the expected item shape. The PostgreSQL layout convention is `/pgdata`, `/pgwal`, `/pgbackup` (created by `tasks/layout.yml`, with the `$PGDATA/backups` symlink that `archive_command` relies on), and memory settings computed from Ansible facts. `postgresql.conf` stays where the distribution puts it (`/etc/postgresql/<v>/main` on Debian, PGDATA on RedHat); `pg_hba.conf` is written into PGDATA on both.
...
- Package repos on RedHat (PGDG `pgdgNN` + `pgdg-common` in `roles/postgresql`, Dalibo Labs in `roles/ldap2pg`) are declared via `yum_repository` with GPG keys vendored in each role's `files/` — bootstrap-RPM downloads through the yum module fail on some guests (`[ASN1: NOT_ENOUGH_DATA]` SSL errors). `prerequisites.yml` enables the CRB repo on RedHat (needed for `perl-IPC-Run` pulled by `libpq-devel`). On Debian, apt keys go to a keyring referenced with `signed-by` (`apt-key` no longer exists on Debian 13).
- On Debian, `createcluster.conf` must keep `start_conf = auto`: with `disabled`, `pg_ctlcluster` refuses to start the cluster ("Cluster is disabled") while the umbrella `postgresql.service` still reports active, so the restart handler cannot see the failure.
- The RedHat PGDATA is set through a systemd drop-in (`/etc/systemd/system/postgresql-NN.service.d/override.conf`), never by editing the RPM-owned unit file.
- Debian and RedHat share `postgresql_initdb_options` (WAL on `/pgwal`, checksums, scram-sha-256): Debian passes them to initdb through `pg_createcluster ... -- <options>`.
- `pg_hba.conf.j2` uses `peer` on the Unix socket and `scram-sha-256` on loopback, matching `postgresql_initdb_options`; role tasks connect as the `postgres` system user through the socket, so no password is needed. pgbouncer uses `auth_type = scram-sha-256` with plaintext passwords in `userlist.txt` (an md5 digest cannot authenticate to a scram-sha-256 server).
- ldap2pg reads `~postgres/ldaprc` and the bind password through its `PASSWORD_FILE` directive (`~postgres/.ldappass`, mode 0600). It does not read `.env` files nor `/etc/ldaprc`.
- On Debian, `postgresql.conf` lives in `/etc/postgresql/<version>/main` (`postgresql_conf_directory`), not in PGDATA like on RedHat, and the `fr_FR.UTF-8` locale must be generated (`conf_manager.yml` does it: `locale_gen` on Debian, `glibc-langpack-fr` on RedHat, whose minimal cloud images only ship the English langpack) or postgres refuses to start with the `lc_*` settings. On Debian, `pg_createcluster` moves `pg_hba.conf`/`pg_ident.conf` to `/etc/postgresql/<v>/main`; the install copies them back into PGDATA because `postgresql_global_config_options` points `hba_file`/`ident_file` there. Service handlers target the instance unit `postgresql@<v>-main`, not the umbrella `postgresql` service, so a failed restart is reported.
